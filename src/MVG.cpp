...
void mvg::reconstructF()
{
   ...
   */
   ...
   if(R2.determinant() < 0)
    ...
    //2.检查svd分解得到的r，t，并进行三角化，选出符合要求的structure和motion
    ...
    if(ngood1 == maxGood)
    {
        ...
        mTriangulateNum = ngood1;
        mR = R1;
        mt = t1;
		mP3d.assign(p3d1.begin(), p3d1.end());
    }
    else if(ngood2 == maxGood)
    {
        mIsTriangulate = vgood2;
        mTriangulateNum = ngood2;
        mR = R1;
        mt = t2;
		mP3d.assign(p3d2.begin(), p3d2.end());
    }
    else if(ngood3 == maxGood)
    {
        mIsTriangulate = vgood3;
        mTriangulateNum = ngood3;
        mR = R2;
        mt = t1;
		mP3d.assign(p3d3.begin(), p3d3.end());
    }
    else if(ngood4 == maxGood)
    {
        mIsTriangulate = vgood4;
        mTriangulateNum = ngood4;
        mR = R2;
        mt = t2;
		mP3d.assign(p3d4.begin(), p3d4.end());
    }
}

// write in ascii
void mvg::savePly(string filename)
{
    ofstream stream(filename.c_str(), std::ios::out | std::ios::binary); 
    if(!stream.is_open())
    {
        cerr<<"stream open error\n";
    }    
    ...
    for(int i = 0; i<n; i++)
    {
        ...
        if(mIsTriangulate[i])
        {
            Point2f pt = mKeypointl_r[i].first.pt;
            size_t blue = mImgl.at<Vec3b>(pt)[0];
            size_t green = mImgl.at<Vec3b>(pt)[1];
            size_t red = mImgl.at<Vec3b>(pt)[2];
		...
		Eigen::Matrix<double, 3, 1> o_r = -mR.transpose() * mt;
        stream
        << o_r(0,0) << ' '
        << o_r(1,0) << ' '
        << o_r(2,0) << ' '
